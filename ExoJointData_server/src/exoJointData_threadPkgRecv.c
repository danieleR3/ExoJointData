...
void* threadPkgRecv(void *arg) 
{
	...
	int numbytes, i = 0, single_joint = 0, buf_id;
    FILE *f;
	...
	while(!readstop()){
		...
		if((*srvParam).sorting){
			if(i==0)
				jointBuf[buf_id]=(JointData *)emalloc((*srvParam).bufSize*sizeof(JointData));
			...
			epthread_mutex_lock(&mtxJointBuf[buf_id]);
				...
				if((nPkgBuf[ANKLE]==(*srvParam).bufSize && nPkgBuf[KNEE]==0) ||
						(nPkgBuf[KNEE]==(*srvParam).bufSize && nPkgBuf[ANKLE]==0)){
				...
				epthread_mutex_lock(&mtxSave);
					if(nPkgBuf[buf_id]==(*srvParam).bufSize){
						saveStarted = 2;
